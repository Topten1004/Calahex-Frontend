...
function PoolComponent(props) {
    ...
    const [poolActivate, setPoolActivate] = React.useState(activate);
    ...
    const ActiveSuccessClose = () => {
        // props.ActivateState(user_id,"pool",true);
        // setPoolActivate(!poolActivate);
        // setActiveModalOpen(false);
        history.push('../twofa',user_id);
    };
    const Pay = async () => {
        handleActiveModalOpen1();
        var res = await props.CreatePayment(user_id, 'activate_pool_'+orderId, selectedCrypto, 20);
        ...
        myInterval = setTimeout(async()=>{
            ...
            if(res == 'done'){
                ...
                history.push('/wallet/pool-account');
            }
        }, 3000)          
    ...
    const InActiveSuccessClose = () => {
        props.ActivateState(user_id,"pool",false);
        setPoolActivate(!poolActivate);
        setInActiveAlertOpen(false);
    };
    const handleTransferState = () => {
        props.ActivateTransferState("Pool");
    }
    ...
    return (
        <Grid item xs={12} sm={6}>
            <div className={classes.exchangeBody}>
                <Typography className={classes.textBody}>Pool Exchange</Typography>
                <Collapse in={hide}>
                ...
                <Typography className={classes.descriptionText}>Receive mining income and
                    participate in a variety of mining financial services and products with Calahex
                    pool.</Typography>
            ...
            <Dialog fullWidth={true} onClose={handleActiveModalClose} aria-labelledby="customized-dialog-title" open={activeModalOpen} >
                ...
                <DialogContent dividers>
                    <Typography gutterBottom>
                        {/* You need to enable 2FA authentication and pay $20 to activate this account . <br/> */}
