...
import { ExchangeWalletState, ExchangeConvert, ExchangeWalletInfo } from "../../../redux/actions/wallet"
...
function HistoryTable(props) {
    ...
    const handleConvert = () => {
        if (price == "") setConvertError(true);
            ...
            if (totalAmount > limitAmount || totalAmount <= 0) setAmountError(true);
                ...
                props.ExchangeConvert(user_id, fromCoin, totalAmount, buyOrderType, toAmount, price);
    ...
    const handleExchangeWalletState = (coinName) => {
        props.ExchangeWalletState(coinName);
    }
                                        ...
                                        return (
                                            ...
                                            >
                                                <Box display={parseFloat(row.available) == 0 && (display == "none") ? "none" : "contents"} >
                                                    ...
                                                    <TableCell align="left" className={classes.actionColor}>
                                                        {/* { ((row.coin==="BTC" || row.coin == "ETH" || row.coin =="USDT"))&&<Button className={classes.actionButton} */}
                                                        {((row.coin === "BTC" || row.coin === "ETH" || row.coin === "USDT" || row.coin === "SXP" || row.coin === "YFI" || row.coin === "UNI" || row.coin === "LINK" || row.coin === "REPV2")) && <Button className={classes.actionButton}
                                                            href={"/wallet/exchange-account/deposit?token=" + row.coin}><span
                                                                className={classes.actionSpan}>Deposit</span></Button>}
                                                        {/* { ((row.coin==="BTC" || row.coin == "ETH" || row.coin =="USDT"))||<Button className={classes.actionButton} */}
                                                        {((row.coin === "BTC" || row.coin === "ETH" || row.coin === "USDT" || row.coin === "SXP" || row.coin === "YFI" || row.coin === "UNI" || row.coin === "LINK" || row.coin === "REPV2")) || <Button className={classes.actionButton}
                                                            href={"/wallet/exchange-account/deposit?token=" + row.coin} disabled><span
                                                                className={classes.actionSpan}>Deposit</span></Button>}
                                                        {/* { ((row.coin==="BTC" || row.coin == "ETH" || row.coin =="USDT") && parseFloat(row.total) > 0  )&&<Button className={classes.actionButton} */}
                                                        {((row.coin === "ETH") && parseFloat(row.available) > 0) && <Button className={classes.actionButton}
                                                            href={"/wallet/exchange-account/withdraw?token=" + row.coin}><span
                                                                className={classes.actionSpan}>Withdraw</span></Button>}
                                                        {/* { ((row.coin==="BTC" || row.coin == "ETH" || row.coin =="USDT") && parseFloat(row.total) > 0 )||<Button className={classes.actionButton} */}
                                                        {((row.coin === "ETH") && parseFloat(row.available) > 0) || <Button className={classes.actionButton}
                                                            href="/wallet/exchange-account/withdraw" disabled><span
                                                                className={classes.actionSpan}>Withdraw</span></Button>}
                                                        {/* <Button className={classes.actionButton}
                                                                                                                            href="/wallet/exchange-account/withdraw" disabled><span
                                                        className={classes.actionSpan}>Withdraw</span></Button> */}
                                                        <Link to="/crypto-exchange" className={classes.transferText}>
                                                            <Button className={classes.actionButton} onClick={() => handleExchangeWalletState(row.coin)}><span className={classes.actionSpan}>Trade</span></Button>
                                                        </Link>
                                                        {(parseFloat(row.available) > 0) && <Button className={classes.actionButton}
                                                            onClick={() => handleActiveModalOpen(row.coin)}><span
                                                                className={classes.actionSpan}>Convert</span></Button>}
                                                        {(parseFloat(row.available) > 0) || <Button className={classes.actionButton}
                                                            onClick={() => handleActiveModalOpen(row.coin)} disabled><span
                                                                className={classes.actionSpan}>Convert</span></Button>}
...
const mapDispatchToProps = {
    ExchangeWalletState, ExchangeConvert, ExchangeWalletInfo
};
