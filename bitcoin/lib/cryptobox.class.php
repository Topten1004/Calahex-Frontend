...
//	     $tmp .= "</div>";
		 ...
		 // Customized
		 $tmp .= "<h3 style='text-align:center'>The payment will be confirmed after a while.</h3>";
	...
	private function check_payment_live()
	{
				...
				isset($res["private_key_hash"]) && strlen($res["private_key_hash"]) == 128 && preg_replace('/[^A-Za-z0-9]/', '', $res["private_key_hash"]) == $res["private_key_hash"] && strtolower($res["private_key_hash"]) == $private_key_hash)
		{
			...
			if ($obj)
			{
				...
				// refresh
				$sql = "UPDATE crypto_payments SET boxType = '".$res["boxtype"]."', amount = ".$res["amount"].", amountUSD = ".$res["amountusd"].", coinLabel = '".$res["coinlabel"]."', unrecognised = 0, addr = '".$res["addr"]."', txDate = '".$res["datetime"]."',txConfirmed = ".$res["confirmed"].", txCheckDate = '".$dt."' WHERE paymentID = ".intval($this->paymentID)." LIMIT 1";
								
				run_sql($sql);

				if($res["confirmed"] == "1"){
					// deposit
					if(strpos($res["order"], 'deposit')!==false)
						print_r("<h3 style='text-align:center;margin-top:20px;margin-bottom:20px;'> Please go back to <a href='https://calahex.com/wallet/exchange-account/deposit'>deposit</a> page.</h3>");

					// activate
					if(strpos($res['order'], 'activate')!==false)
						print_r("<h3 style='text-align:center;margin-top:20px;margin-bottom:20px;'> Please go back to <a href='https://calahex.com/wallet/account-overview'>account overview</a> page.</h3>");
					// sleep(2);
					// // deposit
					// if(strpos($res["order"],'deposit')!==false)
					// 	header("Location: https://calahex.com/wallet/exchange-account/wallet");		
					// // activate					
					// if(strpos($res["order"],'activate')!==false)
			...
			else
			{
				...
				$unrecognised = ($res["status"]=="payment_received_unrecognised"?1:0);
				$datetime = $res["datetime"];
				$confirmed = $res['confirmed'];

				// unrecognised
				if($unrecognised == 1){
					$sql = "INSERT INTO payorders (`user_id`, `payment_id`, `reference`, `unit`, `amount`, `payment_type`, `status`) VALUES ('-1','".intval($this->paymentID)."','".$addr."','".$coinlabel."','".$amount."','unknown','pending')";
					run_sql($sql);
				}

				// deposit
				if(strpos($order, 'deposit')!==false){
					$sql = "INSERT INTO payorders (`user_id`, `payment_id`, `reference`, `unit`, `amount`, `payment_type`, `status`) VALUES ('".$user."','".intval($this->paymentID)."','".explode("_", $order)[1]."','".explode("_", $order)[2]."','".$amount."','deposit','done')";
					run_sql($sql);
				...
				if(strpos($order,'activate_saving')!==false){			
					$sql = "INSERT INTO payorders (`user_id`, `payment_id`, `reference`, `unit`, `amount`, `payment_type`, `status`) VALUES ('".$user."','".intval($this->paymentID)."','saving','USD','".$amountusd."','activate','done')";
					run_sql($sql);
				...
				if(strpos($order,'activate_pool')!==false){			
					$sql = "INSERT INTO payorders (`user_id`, `payment_id`, `reference`, `unit`, `amount`, `payment_type`, `status`) VALUES ('".$user."','".intval($this->paymentID)."','pool','USD','".$amountusd."','activate','done')";
					run_sql($sql);
				...
				if(strpos($order,'activate_margin')!==false){			
					$sql = "INSERT INTO payorders (`user_id`, `payment_id`, `reference`, `unit`, `amount`, `payment_type`, `status`) VALUES ('".$user."','".intval($this->paymentID)."','margin','USD','".$amountusd."','activate','done')";
					run_sql($sql);
	...
	*/
	...
	function run_sql($sql)
	{
        ...
        curl_setopt_array($curl, array(
			CURLOPT_URL => 'https://calahex.io/api/wallet/runSql',
